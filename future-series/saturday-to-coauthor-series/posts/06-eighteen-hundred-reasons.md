...
_Part 6 of From Saturday to Co-Author. [Part 5 covered the cost algebra](/tags/from-saturday-to-coauthor/). This series is dedicated to [Stephen Jordan](https://scholar.google.com/citations?user=dcSsY4cAAAAJ&hl=en&oi=ao)._
...
This post is what was on the other end of that instinct: a seven-layer test architecture, 1875 assertions across 22 files, and one specific bounds check whose absence cost a day in [Part 4](/tags/from-saturday-to-coauthor/) and which now exists as a permanent line in Layer 6.
...
This is the layer that the cost algebra from [Part 5](/tags/from-saturday-to-coauthor/) earns its keep at. The MaxCut anchor and the XORSAT runs go through the *same engine*. Validating the engine on the anchor validates it for the problem family with no anchor of its own, because the only thing that changes between them is the algebra.

### Layer 3: cross-evaluator congruence

This is the layer that catches the subtle bugs.

By the end of the project there were four independent implementations of the same mathematical function: the Basso branch-tensor evaluator from [Part 2](/tags/from-saturday-to-coauthor/); the charge decomposition evaluator from [Part 7](/tags/from-saturday-to-coauthor/) that uses a $\mathbb{Z}_2 \times \mathbb{Z}_2$ character-table decomposition; a reduced-basis variant that exploits tensor symmetry; and a Metal `Float32` GPU forward used only for cross-validation at low $p$.
...
The argument for why this matters is the same shape as the argument in the Tuesday 24 March email from [Part 5](/tags/from-saturday-to-coauthor/): two implementations sharing no code and agreeing to ten digits are evidence of correctness for both, because the only thing they share is the mathematics they are both trying to compute. This is the computational equivalent of two independent experimental groups reproducing each other's result with different equipment.
...
This is the layer whose absence cost the day in [Part 4](/tags/from-saturday-to-coauthor/) at $(k = 7, D = 8)$.
...
This is also the layer the merge script across machines (from [Part 4](/tags/from-saturday-to-coauthor/)) enforces during multi-machine runs.
