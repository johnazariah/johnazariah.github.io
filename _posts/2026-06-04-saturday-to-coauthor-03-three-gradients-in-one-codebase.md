...
This is the standard story for any sufficiently large floating-point pipeline. It is mentioned here for two reasons: because the test harness ([Part 6](/2026/06/15/saturday-to-coauthor-06-eighteen-hundred-reasons.html)) leans on finite differences at $p \le 3$ to validate everything else, and because finite differences are how we *measured* the noise floor, which is how we knew exact gradients were the only path forward.
...
The architectural point is that the evaluator is *closed for modification but open for extension*: adding a gradient strategy did not touch the existing code, only added new code that called into it. Two posts later, this pattern is named ([Part 5](/2026/06/11/saturday-to-coauthor-05-the-algebra-that-runs-itself.html)). Five posts later, the same pattern is used for a different forward pass entirely, with its own adjoint ([Part 7](/2026/06/18/saturday-to-coauthor-07-learning-from-the-masters.html)).
